const apiDataUser = {
    PROJECT_KEY: 'final-task',
    CLIENT_SECRET: 'aeNa5YvB_Y08RfRFSVL0h-j3fhH-4DtM',
    CLIENT_ID: 'pFknkShgF7cMEQWWYLxl355t',
    AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
    API_URL: 'https://api.europe-west1.gcp.commercetools.com',
    SCOPES:
        'create_anonymous_token:final-task view_categories:final-task manage_my_quotes:final-task manage_my_shopping_lists:final-task manage_my_payments:final-task manage_my_orders:final-task view_published_products:final-task manage_my_business_units:final-task manage_my_profile:final-task manage_my_quote_requests:final-task',
};

const apiDataAdm = {
    PROJECT_KEY: 'final-task',
    CLIENT_SECRET: 'qQerccpdYSTY59I4ThUh1eMKHpv8rGs1',
    CLIENT_ID: 'xxBH7Zvs0FeVxCBbRV6JJK4f',
    AUTH_URL: 'https://auth.europe-west1.gcp.commercetools.com',
    API_URL: 'https://api.europe-west1.gcp.commercetools.com',
    SCOPES: 'manage_project:final-task',
};

export { apiDataUser, apiDataAdm };
